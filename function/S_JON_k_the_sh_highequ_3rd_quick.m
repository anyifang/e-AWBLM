function S_k_the_JON_sh_highequ_3rd = S_JON_k_the_sh_highequ_3rd_quick(k,the,U10,H,x_fetch_,C_beta,type,k_store)
% 制作平衡域方法下的浅水高频谱，只考虑3次波相互作用和频散关系
% 低频谱部分采用：浅水波浪谱，Young and Verhagen 1996
% 无量纲谱峰频率
fp_ = 3.5 * x_fetch_^(-0.33);
fp = max(0.13,fp_) * 9.81 / U10; % 谱峰频率
kp = k_calcu_store(fp,H,k_store); % 计算得到水深对应的谱峰波数 通过频散关系反求波束
kp_ = kp * U10^2 / 9.81;
